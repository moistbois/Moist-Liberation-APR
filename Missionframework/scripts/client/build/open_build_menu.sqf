            ...
            if (buildtype != 8) then {
                ...
                switch (_classnamevar) do {
                    ...
                    case KPLIB_b_arsenal: {if (KPLIB_param_mobileArsenal) then {_entrytext = localize "STR_ARSENAL_BOX";};};
                       case (KPLIB_b_mobileRespawn select 0): {if (KPLIB_param_mobileRespawn) then {_entrytext = getText (configFile >> "CfgVehicles" >> (KPLIB_b_mobileRespawn select 0) >> "displayName");};};
                       case (KPLIB_b_mobileRespawn select 1): {if (KPLIB_param_mobileRespawn) then {_entrytext = getText (configFile >> "CfgVehicles" >> (KPLIB_b_mobileRespawn select 1) >> "displayName");};};
                       case (KPLIB_b_mobileRespawn select 2): {if (KPLIB_param_mobileRespawn) then {_entrytext = getText (configFile >> "CfgVehicles" >> (KPLIB_b_mobileRespawn select 2) >> "displayName");};};
